...
card_data_unclean <- read.csv("raw_card_data.csv")

total_sample_size <- length(unique(card_data_unclean$test_taker_id))
```
...
questions_final_df <- find_rule(questions_df) %>%
  select(-c("parsed_config")) # removing column, not needed anymore 
...
card_data_clean <- card_data_expanded %>%
  group_by(test_taker_id) %>%
  filter(n() > 20) %>%
  filter(!test_taker_id %in% consecutive_repeats_df$test_taker_id) %>%
...
categorise_responses <- function(dataframe) {
  unique_ids <- unique(dataframe$test_taker_id)
  
  results <- tibble(
    test_taker_id = unique_ids,
  ...
  for (id in unique_ids) {
    ...
    for (question in 1:nrow(id_data)) {
      ...
      if (is.na(test_taker_answer)) next # Skip iteration if the answer is NA
      ...
      if (question == 1) {
        ...
        if (first_category_complete == FALSE && correct_answer == test_taker_answer) {
          results$trails_first_category[results$test_taker_id == id] <- question
    ...
    results$corr_ans[results$test_taker_id == id] <- corr_ans
    results$p_error[results$test_taker_id == id] <- p_error
    results$non_p_error[results$test_taker_id == id] <- non_p_error
    results$t_error[results$test_taker_id == id] <- t_error
    results$questions_administrated[results$test_taker_id == id] <- corr_ans + t_error
    results$conceptual_level_responses[results$test_taker_id == id] <- consecutive_correct
  }
